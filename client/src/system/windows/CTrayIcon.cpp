...
[[nodiscard]] std::unique_ptr<GUID> make_guid()
{
    ...
    static constexpr std::array<GUID, 1> guids{
        { 0xDA'5F'B2'4F, 0x5C'F9, 0x4C'55, { 0xCB, 0x5D, 0xC6, 0xF9, 0x6B, 0xF8, 0xAA, 0x5E } }
    };
...
[[nodiscard]] std::optional<HANDLE> load_image(HMODULE hModule)
{
    ...
    WIN_CHECK(handle = LoadImage(hModule, MAKEINTRESOURCE(IDI_APPICON), IMAGE_ICON, USE_DEFAULT_XY, USE_DEFAULT_XY, LOAD_FLAGS);
              handle != nullptr);
...
[[nodiscard]] auto create_data(HWND hWindow, const GUID& guid)
{
    return [hWindow, &guid](HANDLE iconHandle) -> std::optional<NOTIFYICONDATAA>
    {
        ...
        return NOTIFYICONDATAA{
            ...
            .szInfoTitle = { 0 },
            .dwInfoFlags =
                NIIF_WARNING | NIIF_USER,    // https://learn.microsoft.com/en-us/windows/win32/api/shellapi/ne-shellapi-shstockiconid
            .guidItem = guid,
...
{
    std::optional<NOTIFYICONDATAA> data =
        get_module_handle().and_then(load_image).and_then(create_data(hWindow, guid)).or_else(log_failure);

    return data;
}
[[nodiscard]] std::optional<NOTIFYICONDATAA> make_notify_con_data_balloon(HWND hWindow,
                                                                          const GUID& guid,
