...
{
    std::vector<float> exitMedian{ -72.0f, -71.0f, -74.0f, -72.0f, -70.0f };
    ...
    for (int j = 0; j < sizes.size(); ++j)
    {
        ...
        for (int i = 0; i < 5; ++i)
        {
            ...
            common::CStopWatch<std::chrono::milliseconds>* timer2{ nullptr };
            bool exit = false;
            while (!exit)
            {
                ...
                if (server.connected())
                {
                    if (timer2 == nullptr)
                    {
                        timer2 = new common::CStopWatch<std::chrono::milliseconds>{};
                    }
