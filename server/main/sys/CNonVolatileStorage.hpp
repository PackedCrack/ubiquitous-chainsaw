...
public:
    ...
    public:
        ...
        [[nodiscard]] std::expected<std::vector<uint8_t>, NvsErrorCode> read_binary(std::string_view key);
        [[nodiscard]] std::expected<int8_t, NvsErrorCode> read_int8(std::string_view key);
    private:
    ...
    public:
        ...
        CWriter(CWriter&& other) = default;
        CWriter& operator=(const CWriter& other) = delete;
        CWriter& operator=(CWriter&& other) = default;
    private:
    ...
    public:
        ...
        [[nodiscard]] NvsErrorCode write_binary(std::string_view key, const std::vector<uint8_t>& data);
        [[nodiscard]] NvsErrorCode write_int8(std::string_view key, int8_t data);
    private:
        [[nodiscard]] NvsErrorCode commit();
    private:
        std::optional<CHandle> m_Handle;
    };    // class CReadWriter
private:
