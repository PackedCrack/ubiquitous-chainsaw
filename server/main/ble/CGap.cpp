...
namespace
{
[[nodiscard]] constexpr std::string_view gap_event_to_str(ble::CGap::Event evnt)
{
    UNHANDLED_CASE_PROTECTION_ON
    switch (evnt)
    {
    case ble::CGap::Event::connect:
        return "connect";
    case ble::CGap::Event::disconnect:
        return "disconnect";
    case ble::CGap::Event::update:
        return "update";
    case ble::CGap::Event::updateReq:
        return "updateReq";
    case ble::CGap::Event::l2capUpdateReq:
        return "l2capUpdateReq";
...
{
    return ble_gap_adv_params{
        // either initilize all or none warning
        .conn_mode = BLE_GAP_CONN_MODE_UND,
        .disc_mode = BLE_GAP_DISC_MODE_GEN,
        .itvl_min = 0,
        .itvl_max = 0,
        .channel_map = 0,
        .filter_policy = 0u,    // whether the advertising packets should be filtered based on the scanner's whitelist (Might be needed for
                                // when we scan to retrieve RSSI)
        .high_duty_cycle =
            0u    // High duty cycle directed advertising increases the frequency of advertising packets sent during directed advertising
    };
}
/// @brief
...
void print_ble_address()
{
    ...
    ble::NimbleErrorCode result =
        ble::NimbleErrorCode{ ble_hs_id_copy_addr(static_cast<uint8_t>(ble::AddressType::randomMac), bleDeviceAddr.data(), nullptr) };
...
[[nodiscard]] ble::AddressType generate_random_device_address()
{
    ...
    result = ble::NimbleErrorCode{
        ble_hs_id_infer_auto(USE_PRIVATE_ADDR, &expectedAddrType)
    };    // 1/private do not work here, type will depend ble_hs_util_ensure_addr()
    if (result != ble::NimbleErrorCode::success)
    {
    ...
    , m_EventCallback{ make_event_callback() }
{
    ...
    {
        auto result = NimbleErrorCode{ ble_svc_gap_device_name_set(
            deviceName.data()) };    // haven't found which error code is returned when this fails
...
{
    int32_t result = ble_gap_adv_start(static_cast<uint8_t>(m_BleAddressType),
                                       nullptr,
                                       BLE_HS_FOREVER,
                                       &m_Params,
                                       CGap::event_callback_caller,
                                       &m_EventCallback);
    ...
    return std::optional<Error>{
        Error{ .code = NimbleErrorCode::unexpectedFailure,
              .msg = "Unknown error received when starting advertising. Return code from nimble: " + std::to_string(result) }
    };
}
/// @brief
/// @return std::nullopt on success. NimbleErrorCode::inProgressOrCompleted if there is no active advertising procedur.
/// NimbleErrorCode::unknown on failure.
std::optional<CGap::Error> CGap::end_advertise()
{
    ...
    {
        return std::optional{
            Error{ .code = NimbleErrorCode::inProgressOrCompleted, .msg = "no active advertising procedure" }
        };
    }
    else
    {
