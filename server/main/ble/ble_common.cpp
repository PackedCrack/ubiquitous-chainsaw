...
std::expected<uint16_t, ble::NimbleErrorCode> chr_attri_handle(uint16_t svcUUID, uint16_t chrUUID)
{
    ...
    std::expected<uint16_t, ble::NimbleErrorCode> expected{};

    // This is one horrible ass API.. I think this is UB tbh.
    auto result = NimbleErrorCode{ ble_gatts_find_chr(reinterpret_cast<const ble_uuid_t*>(&svcUuid),
...
std::expected<std::string, ble::NimbleErrorCode> current_mac_address(AddressType type)
{
    ...
    {
        return FMT("{:02X}:{:02X}:{:02X}:{:02X}:{:02X}:{:02X}", addr[5], addr[4], addr[3], addr[2], addr[1], addr[0]);
    }
    else if (returnCode == NimbleErrorCode::noConfiguredIdentityAddress)
    {
        LOG_WARN_FMT("Could not retrieve Mac address for type: \"{}\", because no address has been configured for that type.",
                     address_type_to_str(type));
    }
