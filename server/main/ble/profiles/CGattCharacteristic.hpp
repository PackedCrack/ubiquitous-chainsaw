...
public:
    ...
    {
        return ble_gatt_chr_def{ .uuid = &(m_Data->uuid.u),
...
public:
    [[nodiscard]] static int
        callback_caller(uint16_t connnectionHandle, uint16_t attributeHandle, ble_gatt_access_ctxt* pContext, function eventCallback)
    {
        const invocable_t& cb = *(static_cast<invocable_t*>(eventCallback));
        return cb(connnectionHandle, attributeHandle, pContext);
    }
    [[nodiscard]] ble_gatt_chr_def to_nimble_t() { return static_cast<ble_gatt_chr_def>(*this); }
private:
    template<typename... flag_t>
    [[nodiscard]] std::unique_ptr<CharacteristicData> make_data(uint16_t uuid, flag_t&&... flags)
    {
        return std::make_unique<CharacteristicData>(
            CharacteristicData{ .flags = (flags | ...), .valueHandle = 0u, .uuid = make_ble_uuid128(uuid) });
    }
...
public:
    template<typename characteristic_t>
    explicit CCharacteristic(characteristic_t&& characteristic)
    requires GattCharacteristic<characteristic_t>
