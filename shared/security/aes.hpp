...
private:
    ...
    void initialize()
    {
        ...
        if (value)
        {
            ...
            rng.generate_block(aes_mode::BLOCK_SIZE)
                .and_then(
                    [this](const std::vector<uint8_t>& dataBlock) -> std::expected<std::vector<uint8_t>, CRandom::Error>
                    {
                        ASSERT(valid_key_size(m_Passphrase), "The provided passphrase must be 16, 24 or 32 bytes long!");
                ...
                .or_else(
                    [](CRandom::Error err) -> std::expected<std::vector<uint8_t>, CRandom::Error>
                    {
    ...
    [[nodiscard]] bool valid_key_size(std::string_view key) { return key.size() == 16u || key.size() == 24u || key.size() == 32u; }
private:
    Aes m_Decryptor;
    Aes m_Encryptor;
